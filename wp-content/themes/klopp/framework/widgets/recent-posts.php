...
class klopp_Recent_Posts extends WP_Widget {

	public function __construct() {
		parent::__construct(
			'klopp_rp', // Base ID
			__('Klopp Recent Posts with Thumbnails', 'klopp'), // Name
			array( 'description' => __( 'Display your recent posts, with a Thumbnail.', 'klopp' ), ) // Args
		);
	}
	...
	if (isset($instance['title'])) :
		...
		?>
		      ...
		      <?php _e('Oops, there are no posts.','klopp'); ?>
 	...
 	public function form( $instance ) {
		if ( isset( $instance[ 'title' ] ) ) {
			...
			$title = __( 'Latest Articles', 'klopp' );
		}
		if ( isset( $instance[ 'no_of_posts' ] ) ) {
			$no_of_posts = $instance[ 'no_of_posts' ];
		}
		else {
			$no_of_posts = __( '5', 'klopp' );
		}
		?>
		<p>
		<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:','klopp' ); ?></label> 
		<input class="widefat" id="<?php echo esc_attr($this->get_field_id( 'title' )); ?>" name="<?php echo esc_attr($this->get_field_name( 'title' )); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		
		<label for="<?php echo esc_attr($this->get_field_id( 'no_of_posts' )); ?>"><?php _e( 'No. of Posts:', 'klopp' ); ?></label>
...
add_action( 'widgets_init', 'register_klopp_widget' );  
function register_klopp_widget() {  
    register_widget( 'klopp_Recent_Posts' );  
}  
