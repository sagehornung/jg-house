...
class Nova_Config extends Nova_Customizer {
	...
	public function __construct( $id, $args = array() ) {
		...
		// Process the 'nova/config' filter by replacing the defaults
	...
	/**
	 * Adds the configuration to the Nova object.
	 *
	...
	public function add_config( $config_id, $args ) {
		...
		// Set the config
		Nova::$config[ $config_id ] = array_merge( $this->default_args, $args );
	}

	/**
	 * Parses the 'nova/config' filter.
	 *
	 * @return  array
	 */
	public function config_from_filters() {
		// get the args from the filter
		$args = apply_filters( 'nova/config', $this->default_args );
