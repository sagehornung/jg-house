...
if ( class_exists( 'Nova_Google_Fonts_Registry' ) ) {
	return;
}

class Nova_Google_Fonts_Registry {
	...
	public function get_all_fonts() {
		...
		return apply_filters( 'nova/fonts/all', array_merge( $standard_fonts, $google_fonts ) );
	}
	...
	public function choose_google_font_variants( $font, $variants = array() ) {
		...
		return apply_filters( 'nova/font/variants', array_unique( $chosen_variants ), $font, $variants );
	...
	public function get_standard_fonts() {
		...
		if ( null == $this->standard_fonts ) {
			$this->standard_fonts = apply_filters( 'nova/fonts/standard_fonts', array(
				'serif'     => array(
					'label' => $i18n['serif'],
	...
	public function get_google_fonts() {
		...
		if ( null == $this->google_fonts ) {
			...
			$json = $wp_filesystem->get_contents( Nova::$path . DIRECTORY_SEPARATOR . 'assets' . DIRECTORY_SEPARATOR . 'json' . DIRECTORY_SEPARATOR . 'webfonts.json' );
			...
			$this->google_fonts = apply_filters( 'nova/fonts/google_fonts', $google_fonts );
