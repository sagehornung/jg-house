...
class Nova_Values {
	...
	public static function get_value( $config_id = '', $field_id = '' ) {
		...
		if ( 'theme_mod' == Nova::$config[ $config_id ]['option_type'] ) {
			/**
			 * We're using theme_mods.
			 * so just get the value using get_theme_mod
			 */
			$default_value = null;
			if ( isset( Nova::$fields[ $field_id ] ) && isset( Nova::$fields[ $field_id ]['default'] ) ) {
				$default_value = Nova::$fields[ $field_id ]['default'];
			}
			 ...
			 */
			if ( isset( Nova::$fields[ $field_id ] ) && isset( Nova::$fields[ $field_id ]['type'] ) && 'background' == Nova::$fields[ $field_id ]['type'] ) {
				$value = array();
		...
		} elseif ( 'option' == Nova::$config[ $config_id ]['option_type'] ) {
			/**
			 * We're using options.
			 */
			if ( '' != Nova::$config[ $config_id ]['option_name'] ) {
				/**
				 * Options are serialized as a single option in the db.
				 * We'll have to get the option and then get the item from the array.
				 */
				$options = get_option( Nova::$config[ $config_id ]['option_name'] );

				if ( ! isset( Nova::$fields[ $field_id ] ) && isset( Nova::$fields[ Nova::$config[ $config_id ]['option_name'] . '[' . $field_id . ']'] ) ) {
					$field_id = Nova::$config[ $config_id ]['option_name'] . '[' . $field_id . ']';
				}
				$setting_modified = str_replace( ']', '', str_replace( Nova::$config[ $config_id ]['option_name'] . '[', '', $field_id ) );

				/**
				 * If this is a background field, get the individual sub-fields and return an array.
				 */
				if ( 'background' == Nova::$fields[ $field_id ]['type'] ) {
					$value = array();

					foreach ( Nova::$fields[ $field_id ]['default'] as $property => $property_default ) {
					 ...
					 */
					$value = ( isset( $options[ $setting_modified ] ) ) ? $options[ $setting_modified ] : Nova::$fields[ $field_id ]['default'];
				 ...
				 */
				$value = get_option( $field_id, Nova::$fields[ $field_id ]['default'] );
				...
				if ( 'background' == Nova::$fields[ $field_id ]['type'] ) {
					$value = array();
					foreach ( Nova::$fields[ $field_id ]['default'] as $property_key => $property_default ) {
		...
		return apply_filters( 'nova/values/get_value', $value, $field_id );
