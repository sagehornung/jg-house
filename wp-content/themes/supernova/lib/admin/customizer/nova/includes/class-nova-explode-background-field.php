...
class Nova_Explode_Background_Field extends Nova_Field_Sanitize {
	/**
	...
	public static function explode( $field ) {
		...
		foreach ( $field['default'] as $key => $value ) {
			...
			$fields[ $property_setting ] = array_merge( $field, array(
				...
				'default'     => $value,
				'id'          => Nova_Field_Sanitize::sanitize_id( array( 'settings' => Nova_Field_Sanitize::sanitize_settings( array( 'settings' => $field['settings'] . '_' . $setting ) ) ) ),
				...
				'sanitize_callback' => ( isset( $sanitize_callback ) ) ? $sanitize_callback : Nova_Field_Sanitize::fallback_callback( $type ),
			) );
			$i++;
		}
