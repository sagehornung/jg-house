...
if ( class_exists( 'Nova_Colourlovers' ) ) {
	return;
}

class Nova_Colourlovers {

	/**
	 * Returns an array properly formatted for use by the Nova_Palette control.
	 *
	...
	public static function get_palettes( $palettes_nr = 5 ) {
		...
		foreach ( $palettes as $palette ) {
			...
			foreach ( $palette as $key => $value ) {
				$palettes[$i][$key] = Nova_Color::sanitize_hex( $value );
			}
			$i++;
		}
	...
	public static function parse( $xml = null ) {
		 ...
		 */
		$xml_url  = ( is_null( $xml ) ) ? trailingslashit( Nova::$url ) . 'assets/xml/colourlovers-top.xml' : $xml;
