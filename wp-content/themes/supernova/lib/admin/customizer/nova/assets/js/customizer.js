 ...
 */
wp.customize.controlConstructor['nova-checkbox'] = wp.customize.Control.extend( {
	ready: function() {
		var control = this;
...
wp.customize.controlConstructor['code'] = wp.customize.Control.extend( {
	ready: function() {
		var control = this;
		var element = control.container.find( '#nova-codemirror-editor-' + control.id );
...
jQuery(document).ready(function($) {
	...
	if ( typeof Color !== "undefined" ) {
		...
		$('.nova-color-control').each(function() {
			var $control = $(this),
			...
			$('<div class="nova-alpha-container"><div class="slider-alpha"></div><div class="transparency"></div></div>').appendTo($control.parents('.wp-picker-container'));
			...
			jQuery.each( control.params.choices, function( key, value ) {
				...
				if ( select_value == key ) {
					...
					jQuery.each( value['settings'], function( preset_setting, preset_setting_value ) {
						 ...
						 *     nova-checkbox
						 */
						if ( 'checkbox' == sub_control_type || 'switch' == sub_control_type || 'toggle' == sub_control_type || 'nova-checkbox' == sub_control_type ) {
						 ...
						 *     nova-select
						 */
						else if ( 'select' == sub_control_type || 'select2' == sub_control_type || 'select2-multiple' == sub_control_type || 'nova-select' == sub_control_type ) {
						 ...
						 */
							 ...
							 */
							var numeric_element = wp.customize.control( preset_setting ).container.find( '.nova_range_value .value' );
						 ...
						 *     nova-textarea
						 */
						else if ( 'textarea' == sub_control_type || 'nova-textarea' == sub_control_type ) {
						 ...
						 *     nova-color
						 */
						else if ( 'color' == sub_control_type || 'nova-color' == sub_control_type ) {
							 ...
							 * Update the value visually in the control
							 */
							var alphaColorControl = wp.customize.control( preset_setting ).container.find( '.nova-color-control' );
						 ...
						 *     nova-radio
						 */
						else if ( 'radio-buttonset' == sub_control_type || 'radio-image' == sub_control_type || 'radio' == sub_control_type || 'nova-radio' == sub_control_type ) {
...
wp.customize.controlConstructor['repeater'] = wp.customize.Control.extend({
    ready: function() {
        ...
        // The current value set in Control Class (set in Nova_Customize_Repeater_Control::to_json() function)
    ...
    addRow: function( data ) {
        ...
        // The template for the new row (defined on Nova_Customize_Repeater_Control::render_content() )
 ...
 */
function novaArrayToObject( arr ) {
	var rv = {};
	if ( null !== arr ) {
...
wp.customize.controlConstructor['nova-select'] = wp.customize.Control.extend( {
	ready: function() {
		var control = this;
		...
		this.container.on( 'change', 'select', function() {
			if ( multiple > 1 ) {
				var select_value = novaArrayToObject( jQuery( this ).val() );
			} else {
				var select_value = jQuery( this ).val();
			}
			control.setting.set( select_value );
...
jQuery(document).ready(function($) {
	...
	$( 'input[type=range]' ).on( 'mousedown', function() {
		...
		$( this ).mousemove(function() {
			value = $( this ).attr( 'value' );
			$( this ).closest( 'label' ).find( '.nova_range_value .value' ).text( value );
		});
	});

	$( '.nova-slider-reset' ).click( function () {
